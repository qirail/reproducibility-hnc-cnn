...
class ImageClassificationBase(nn.Module):
    ...
    def validation_step(self, batch, class_weights, predict=False):
        """ Perform a validation step (now with ROC-AUC 95% CI)"""
        ...
        # Move to CPU numpy for sklearn + bootstrap
        y_true = labels.detach().cpu().numpy()
        y_score = predictions.detach().cpu().numpy().ravel()
        ...
        # === NEW: ROC-AUC bootstrap CI (dict with LOWER_BOUND/HIGHER_BOUND) ===
        ci_dict = compute_ci(y_score, y_true, samples=1000, sig_level=0.05)
        
        # Precision-Recall curve and respective AUC
        # precision, recall, thresholds_pr = metrics.precision_recall_curve(labels, predictions) # This works only if they happen to be on CPU.
        precision, recall, thresholds_pr = metrics.precision_recall_curve(y_true, y_score)
        ...
        performance = {
            ...
            ROC: {
                ...
                TPR: tpr[threshold],
                FPR: fpr[threshold],
                CI: ci_dict,  # <- added CI dict (LOWER_BOUND/HIGHER_BOUND etc.)
            },
            PR: {
