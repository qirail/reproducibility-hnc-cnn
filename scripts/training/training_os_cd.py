...
CONFIGURATIONS = {
    ...
    LOGS_PATH: "results/logs/log_os_cd_ANN.txt",  # Directory to store log files
...
random.seed(3863768) # Set a fixed seed for reproducibility
# 2) Random split seed
random_seed_split = random.randint(0, 4843071) # Random seed for splitting data into folds
# 3) Random seed torch
torch.manual_seed(3381915) # Set seed for reproducibility in PyTorch
            ...
            with open(file_path, "w") as o:
                ...
                with contextlib.redirect_stdout(o):
                    ...
                    history = fit(
                        ...
                        dataloaders,                      # Dictionary of all dataloaders (train/val/test)
                        parameters=CONFIGURATIONS[HYPERPARAMETERS],  # Hyperparameters such as learning rate
                        store_model={**CONFIGURATIONS[STORE_MODEL], MODEL_ID: f"os_ANN_5-fold_cv_fold{fold}"},  # Configuration for saving the trained model
                    )
